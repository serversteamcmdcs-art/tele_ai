    ...
    <div className="flex-1 flex flex-col h-full">
      ...
      <div className="h-14 px-4 flex items-center justify-between border-b border-white/[0.06] glass flex-shrink-0">
        <div className="flex items-center gap-3">
          {onBack && (
            <button
              onClick={onBack}
              className="md:hidden w-8 h-8 rounded-full bg-white/[0.06] flex items-center justify-center hover:bg-white/[0.1] transition-colors text-gray-400 hover:text-white mr-1"
            >
              <ArrowLeft className="w-5 h-5" />
            </button>
          )}
          {activeChat?.type === 'private' && otherUser?.avatarUrl ? (
            <img 
              src={otherUser.avatarUrl} 
              alt="" 
              className="w-10 h-10 rounded-full object-cover ring-2 ring-acid-cyan/30 cursor-pointer hover:ring-acid-cyan/50 transition-all"
              onClick={() => otherUser?.id && setShowUserProfile(otherUser.id)}
            />
          ) : (
          ...
          <div>
            ...
            <div className="text-xs text-gray-400 flex items-center gap-1.5">
              {otherUser?.isOnline ? (
                <span className="text-acid-green flex items-center gap-1.5">
        ...
        <div className="flex items-center gap-1">
          ...
          {(activeChat?.type === 'group' || activeChat?.type === 'channel') && (
            <button
              ...
              className="p-2 rounded-lg hover:bg-white/[0.06] transition-colors text-gray-400 hover:text-acid-green"
      ...
      {showAddMembers && activeChat && (
        <AddMembersModal
          ...
          onMembersAdded={() => {
            // Обновляем список чатов для получения актуального числа участников
