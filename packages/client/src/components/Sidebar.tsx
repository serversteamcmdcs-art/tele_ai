...
export function Sidebar() {
  ...
  return (
    <>
      <div className="w-full md:w-80 h-full flex flex-col border-r border-white/[0.06] glass relative z-10">
        {/* Шапка */}
              ...
              return (
                ...
                {/* Аватар */}
                   <div className="relative flex-shrink-0">
                     {chat.type === 'private' && otherUser?.avatarUrl ? (
                       <img 
                         src={otherUser.avatarUrl} 
                         alt="" 
                         className="w-12 h-12 rounded-full object-cover ring-2 ring-white/[0.1] cursor-pointer hover:ring-acid-cyan/50 transition-all"
                         onClick={(e) => {
                           e.stopPropagation();
                           if (otherUser?.id) setShowUserProfile(otherUser.id);
                         }}
                       />
                     ) : (
                       <div 
                         className={`w-12 h-12 rounded-full flex items-center justify-center text-white font-medium cursor-pointer ${
                           chat.type === 'saved'
                         ...
                         onClick={(e) => {
                           e.stopPropagation();
                           if (chat.type === 'private' && otherUser?.id) {
                             setShowUserProfile(otherUser.id);
                           }
                         }}
                       >
