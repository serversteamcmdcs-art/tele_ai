...
export function NewChatModal({ onClose }: Props) {
  ...
  const { createPrivateChat, createGroup, createChannel, setActiveChat } = useChatStore();
        ...
        <div className="p-4 space-y-3">
          ...
          {tab === 'channel' && (
            <textarea
              ...
              rows={2}
              className="acid-input w-full px-4 py-2.5 bg-black/30 border border-white/[0.06] rounded-xl text-white placeholder-gray-500 text-sm resize-none"
            />
          )}

          {/* Выбранные пользователи */}
          {(tab === 'group' || tab === 'channel') && selectedUsers.length > 0 && (
          ...
          {/* Поиск */}
          {(tab === 'private' || tab === 'group' || tab === 'channel') && (
            <>
