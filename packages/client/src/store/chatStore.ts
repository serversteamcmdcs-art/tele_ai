...
interface ChatState {
  ...
  createChannel: (title: string, description: string, participantIds: string[]) => Promise<any>;
  addMembersToChat: (chatId: string, userIds: string[]) => Promise<any>;
...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  createChannel: async (title, description, participantIds) => {
    const res = await api.createChannel(title, description, participantIds);
    if (res.success && res.data) {
      await get().loadChats();
      return res.data;
    }
  ...
  addMembersToChat: async (chatId, userIds) => {
    const res = await api.addMembersToChat(chatId, userIds);
    if (res.success && res.data) {
      await get().loadChats();
      return res.data;
    }
  ...
  initWsListeners: () => {
    ...
    unsubs.push(wsClient.on('message:new', (data) => {
      ...
      if (activeChat && data.chatId === activeChat.id) {
        ...
        if (!alreadyExists) {
          // Добавляем информацию об отправителе из активного чата
          const sender = activeChat.participants?.find((p: any) => p.id === data.senderId);
          const enrichedData = {
            ...data,
            sender: data.sender || sender || { id: data.senderId },
          };
          set({ messages: [...messages, enrichedData] });
        }
    ...
    unsubs.push(wsClient.on('user:online', (data) => {
      ...
      set({
        ...
        activeChat: activeChat ? {
          ...activeChat,
          participants: activeChat.participants?.map((p: any) =>
            p.id === data.userId ? { ...p, isOnline: true } : p,
          ),
        } : null,
      });
