...
class ApiClient {
  ...
  blockUser(userId: string) {
    return this.request('POST', '/users/me/block', { userId });
  }

  unblockUser(userId: string) {
    return this.request('DELETE', `/users/me/block/${userId}`);
  }
  ...
  createChannel(title: string, description?: string, participantIds?: string[]) {
    return this.request<any>('POST', '/chats/channel', { title, description, participantIds });
  }

  addMembersToChat(chatId: string, userIds: string[]) {
    return this.request<any>('POST', `/chats/${chatId}/members`, { userIds });
  }
