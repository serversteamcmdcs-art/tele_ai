...
export async function chatRoutes(app) {
    ...
    app.post('/group', {
        ...
        const { title, participantIds } = request.body;
        if (!title) {
    ...
    app.post('/:chatId/messages', {
        ...
        const { chatId } = request.params;
        const { text, type = 'text', replyToId, mediaUrl, mediaType, mediaName, mediaSize } = request.body;
        const membership = await prisma.chatMember.findUnique({
            where: { chatId_userId: { chatId, userId: request.user.id } },
        });
        if (!membership) {
    ...
    app.post('/:chatId/messages/:messageId/pin', {
        ...
        if (existing) {
            ...
            sendToChat(chatId, { type: 'message:unpinned', data: { chatId, messageId } });
    ...
    app.post('/join/:inviteLink', {
        ...
        const { inviteLink } = request.params;
