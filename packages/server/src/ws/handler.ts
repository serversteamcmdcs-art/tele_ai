...
async function handleSendMessage(userId: string, data: any) {
  ...
  // Для личных чатов проверяем блокировку
  ...
  if (chat?.type === 'private') {
    ...
    if (otherUserId) {
      ...
      if (blocked) {
        // Пользователь заблокирован, не отправляем сообщение
        return;
      }
    }
  }
...
async function broadcastUserStatus(userId: string, isOnline: boolean) {
  // Получаем всех пользователей, с которыми есть общие чаты
  ...
  // Находим всех участников этих чатов
  ...
  // Уведомляем всех участников (кроме самого пользователя)
  for (const member of allMembers) {
    if (member.userId !== userId && !notifiedUsers.has(member.userId)) {
      notifiedUsers.add(member.userId);
      sendToUser(member.userId, isOnline
