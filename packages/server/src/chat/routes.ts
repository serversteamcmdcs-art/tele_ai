...
import { sendToChat, sendToUser } from '../ws/handler.js';
...
export async function chatRoutes(app: FastifyInstance) {
  ...
  app.post('/channel', {
    ...
    const { title, description, participantIds } = request.body as {
      title: string;
      description?: string;
      participantIds?: string[];
    };
  ...
  app.post('/:chatId/members', {
    preHandler: [app.authenticate as any],
  }, async (request: any, reply) => {
    const { chatId } = request.params as { chatId: string };
    const { userIds } = request.body as { userIds: string[] };

    // Проверить, что пользователь является участником с правами приглашения
    const membership = await prisma.chatMember.findUnique({
      where: { chatId_userId: { chatId, userId: request.user.id } },
    });
    ...
    // Для каналов и групп проверяем права на приглашение
    ...
    // Только owner и admin могут добавлять участников в каналы
    if (chat.type === 'channel' && !['owner', 'admin'].includes(membership.role)) {
      return reply.status(403).send({ success: false, error: 'Недостаточно прав для добавления участников' });
    }

    if (!userIds || userIds.length === 0) {
      return reply.status(400).send({ success: false, error: 'Укажите пользователей для добавления' });
    }
    ...
    // Отправить уведомления добавленным пользователям
    ...
    // Получить обновлённый список участников
    const members = await prisma.chatMember.findMany({
      where: { chatId },
      include: {
        user: {
    ...
    return reply.send({ success: true, data: members, added: newMembers.count });
  });
